...
public final class DisketteMatcher implements GearMatcher {
	...
	public Confidence matches(final GearContext context) {
		final boolean hasTrackDensity = context.getFacts(AttributeNames.TRACK_DENSITY, TrackDensity.class)
				.isPresent();
		final boolean hasFloppyDiskFormat = context
				.getFacts(AttributeNames.FLOPPY_DISK_FORMAT, FloppyDiskFormat.class).isPresent();
