...
public interface FileNameClueFinder extends ClueFinder {
	...
	/**
	 * Paths are guaranteed to represent files and are relative to their configured
	 * archive root. Finders should retain this relative form when a clue refers to
	 * an archive file.
	 */
	Set<Clue> find(Collection<Path> files);

	/**
	 * Serializes an archive-relative path with a stable separator for a portable
	 * clue cache.
	 */
