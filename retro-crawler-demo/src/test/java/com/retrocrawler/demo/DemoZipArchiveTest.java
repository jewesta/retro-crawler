...
class DemoZipArchiveTest {
	...
	void crawlsThePackagedRetroPcZipWithoutExtraction() throws Exception {
		...
		final Model model = Model.from(DemoModels.RETRO_PC.getBasePackage(), ArchiveRoots.from(zip));
		final RetroCrawler crawler = RetroCrawler.builder().model(model).repository(new InMemoryRepository())
				.archiveSource(new ZipArchiveSource()).build();

		final List<MyKnownGear> gear = crawler.crawlGear(new Progressor(), ReindexScope.all(), MyKnownGear.class);
